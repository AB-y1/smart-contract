...
def add_text(image, text, position, font_size=500, color=(0, 0, 0), font_path=None):
    ...
    x = position[0] - (text_bbox[1] - text_bbox[1]) // 2
...
background_with_name = add_text(background.copy(), full_name, (background.shape[1] // 2.8, 640))

# Add Date to the certificate background
background_with_date = add_text(background_with_name.copy(), date, (background.shape[1] //2.8, 716))
